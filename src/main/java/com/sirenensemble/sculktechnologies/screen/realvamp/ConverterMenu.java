...
public class ConverterMenu extends AbstractContainerMenu {
    public static ExpSculkConverterEntity blockEntity;
    private final Level level;
    private final ContainerData data;


    public ConverterMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
    }

    public ConverterMenu(int containerId, Inventory inv, BlockEntity blockEntity, ContainerData data) {
        super(ModMenuTypes.CONVERTER_MENU.get(), containerId);
        this.blockEntity = ((ExpSculkConverterEntity) blockEntity);
        this.level = inv.player.level();
        this.data = data;
        addPlayerInventory(inv);
        addPlayerHotbar(inv);
        this.addSlot(new SlotItemHandler(this.blockEntity.INVENTORY, 0, 80, 16));
        this.addSlot(new SlotItemHandler(this.blockEntity.INVENTORY, 1, 80, 54){
            @Override
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, ModBlock.EXPSCULKCONVERTER.get());
    }
